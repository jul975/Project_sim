...
@dataclass
class DailyAgentFlux:
    distance_m: float = 0.0
    intake_raw: float = 0.0
...
def maintenance_phase(
    state, params, dt, branch_budget: BranchBudget
) -> MaintenanceResult:
    ...
    branch_budget_after_maturity = max(
        branch_budget.maturity_repro_budget_J - p_J * dt, 0.0
    )
    ...
    return MaintenanceResult(
        somatic_maintenance_paid_J=p_S * dt, maturity_maintenance_paid_J=p_J * dt
    )
...
def interrection_phase(agent, soma_surplus, params, dt) -> MovementResult:
    """Determine energy-limited movement capacity and deduct actual movement cost."""
    ## needs implementation
    ## note, will work around max daily range of 9600m, will need movement distance correction unit for now, => more complex movement has to weight.
...
def growth_phase(
    state: EnergyState, soma_surplus_after_movement, params, dt
) -> GrowthResult:
    ...
    dV = (
        structural_growth_rate_cm3_per_day(
            soma_surplus_after_movement, params, movement_power_j
        )
        * dt
    )
